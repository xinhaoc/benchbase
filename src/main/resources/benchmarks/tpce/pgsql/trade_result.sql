...
CREATE OR REPLACE FUNCTION TradeResultFrame1(IN trade_id BIGINT)
    RETURNS record AS
$$
DECLARE
    -- output parameters
...
BEGIN
    ...
    IF type_is_sell THEN
        ...
        IF holdsum_qty = 0 THEN
            ...
            INSERT INTO HOLDING_SUMMARY (HS_CA_ID,
                                         HS_S_SYMB,
                                         HS_QTY)
            ...
            WHILE needed_qty > 0
                LOOP
                    ...
                    IF hold_qty > needed_qty THEN
                        ...
                        INSERT INTO HOLDING_HISTORY (HH_H_T_ID,
                                                     HH_T_ID,
                                                     HH_BEFORE_QTY,
                                                     HH_AFTER_QTY)
...
CREATE OR REPLACE FUNCTION TradeResultFrame3(
    ...
    IN trade_id BIGINT,
    IN tax_amnt FLOAT) RETURNS FLOAT AS
$$
DECLARE
...
CREATE OR REPLACE FUNCTION TradeResultFrame5(
    ...
    IN trade_id BIGINT,
    IN trade_price FLOAT) RETURNS integer AS
$$
DECLARE
...
BEGIN
    ...
    INSERT INTO TRADE_HISTORY (TH_T_ID,
                               TH_DTS,
                               TH_ST_ID)
...
CREATE OR REPLACE FUNCTION TradeResultFrame6(
    ...
    IN trade_qty INTEGER,
    IN type_name char(12)) RETURNS FLOAT AS
$$
DECLARE
    -- Local Frame Variables
    cash_type char(40);
