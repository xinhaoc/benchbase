...
BEGIN
    ...
    ELSIF table_name = 'DAILY_MARKET' THEN
        ...
        WHERE dm_s_symb = symbol
          AND SUBSTRING(CAST(DM_DATE AS TEXT) FROM 6 FOR 2)::integer = day_of_month;
    ELSIF table_name = 'EXCHANGE' THEN
        ...
        ELSE
            UPDATE EXCHANGE
            SET EX_DESC = substring(EX_DESC from 1 for char_length(EX_DESC) - char_length(CAST(now() AS TEXT))) || now();
--SET	EX_DESC = substring(EX_DESC,1,len(EX_DESC)-len(getdatetime())) + getdatetime();
    ...
    ELSIF table_name = 'FINANCIAL' THEN
        ...
        WHERE FI_CO_ID = in_co_id
          AND substring(CAST(FI_QTR_START_DATE AS TEXT) from 9 for 2)::smallint = 1;
